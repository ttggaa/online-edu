var Index = function () {
    ...
    return {
        ...
        initCalendar: function (calendarJsonStr) {
...
//                editable: true,
                events: calendarJsonStr
            });
        },

        initCharts: function (json) {
            if (!jQuery.plot) {
                return;
            }
            ...
            function showTooltip2(title, x, y, hour, count) {
                $('<div id="tooltip" class="chart-tooltip"><div class="date">' + title + '<\/div><div class="label label-success">时: ' + hour + '<\/div><div class="label label-important">访问量: ' + count + '<\/div><\/div>').css({
                    position: 'absolute',
                    display: 'none',
                    top: y - 100,
                    width: 75,
            ...
            var pageviews = json.data.dayAccessPageviews;
            var pageviews2 = json.data.yesterdayAccessPageviews;
            //alert(pageviews);
                ...
                $("#site_statistics").bind("plothover", function (event, pos, item) {
                    ...
                    if (item) {
                        if (previousPoint != item.dataIndex) {
                            ...
                            showTooltip2('访问量统计', item.pageX, item.pageY,  x , y);
                        }
                    } else {
                        $("#tooltip").remove();
                        previousPoint = null;
